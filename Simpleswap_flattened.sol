...
require(_tokenA == tokenA && _tokenB == tokenB, "Invalid token pair");   //Verify that the tokens are correct.
...
IERC20(tokenA).transferFrom(msg.sender,address(this),amountADesired); //  "Transfer tokens from the user to Simpleswap
IERC20(tokenB).transferFrom(msg.sender,address(this),amountBDesired);
 
uint newBalanceA = IERC20(tokenA).balanceOf(address(this)); // Here I check how many tokens I currently have.
uint newBalanceB = IERC20(tokenB).balanceOf(address(this));
 
 
 
amountA = newBalanceA - balanceA; //And this is the actual amount I received
...
require(block.timestamp <= deadline, "Deadline expired");  //check DeanLine
require(_tokenA == tokenA && _tokenB == tokenB, "Invalid token pair");   //Verify that the tokens are correct.
...
require(totalLiquidity > 0, "No liquidity available");
 ...
 uint reserveB = IERC20(tokenB).balanceOf(address(this));
        ...
        // Calculate the proportion of tokens to withdraw
        ...
        // Transfer of tokens A and B to the user.
        _burn(msg.sender, liquidity);
 
        // Transfer de tokens from  A  B to  user
...
require(
    ...
    // Transfer tokens A and B to the user
    ...
    // Calculate amountOut using the Uniswap v2 formula: 
    
    uint amountOut = (amountIn * reserveOut) / (reserveIn + amountIn);
    require(amountOut >= amountOutMin, "Insufficient output amount");
 
    // Transfer output token to the recipient
